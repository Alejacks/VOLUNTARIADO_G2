...
--CREACIÓN DE TABLAS CON CONSTRAINTS, REGLAS, ÍNDICES, ETC...
...
--CUMPLE_CON
CREATE PROCEDURE ASOCIAR_CUMPLE_CON
@ID_ACTIVIDAD SMALLINT,
@ID_ODS SMALLINT
AS 
INSERT INTO CUMPLE_CON (ID_ACTIVIDAD, ID_ODS) VALUES (@ID_ACTIVIDAD, @ID_ODS)
GO

--REALIZA
CREATE PROCEDURE ASOCIAR_REALIZA
@ID_ACTIVIDAD SMALLINT,
@NIF_VOLUNTARIO CHAR (10)
AS 
INSERT INTO REALIZA (ID_ACTIVIDAD, NIF_VOLUNTARIO) VALUES (@ID_ACTIVIDAD, @NIF_VOLUNTARIO)
GO

--INTERES
CREATE PROCEDURE ASOCIAR_INTERES
@ID_TIPO SMALLINT,
@NIF_VOLUNTARIO CHAR(10)
AS
INSERT INTO INTERES (ID_TIPO, NIF_VOLUNTARIO) VALUES (@ID_TIPO, @NIF_VOLUNTARIO)
GO

--ES_DE_TIPO
CREATE PROCEDURE ASOCIAR_ES_DE_TIPO
@ID_TIPO SMALLINT,
@ID_ACTIVIDAD SMALLINT
AS
INSERT INTO ES_DE_TIPO (ID_TIPO, ID_ACTIVIDAD) VALUES (@ID_TIPO,@ID_ACTIVIDAD)
GO

--DISPONE_DE
CREATE PROCEDURE ASOCIAR_DISPONE_DE
@NIF_VOLUNTARIO CHAR(10),
@DIA CHAR(9),
@HORA_INICIO TIME,
@HORA_FIN TIME
AS
INSERT INTO DISPONE_DE (NIF_VOLUNTARIO,DIA,HORA_INICIO,HORA_FIN)VALUES (@NIF_VOLUNTARIO,@DIA,@HORA_INICIO,@HORA_FIN)
GO

--TRIGGERS PARA ELIMINACIONES

CREATE TRIGGER AL_ELIMINAR_VOLUNTARIO ON VOLUNTARIO
INSTEAD OF DELETE
AS
BEGIN
    DELETE DISPONE_DE
    WHERE NIF_VOLUNTARIO IN (SELECT NIF FROM DELETED);
    ...
    AND NOT EXISTS (
        ...
        FROM DISPONE_DE
        WHERE DIA = HORA.DIA
        AND HORA_INICIO = HORA.HORA_INICIO
        AND HORA_FIN = HORA.HORA_FIN
        AND NIF_VOLUNTARIO NOT IN (SELECT NIF FROM DELETED)
    )
...
CREATE TRIGGER AL_ELIMINAR_CURSO ON CURSO
INSTEAD OF DELETE
AS
BEGIN
    DELETE VOLUNTARIO WHERE ESTUDIA IN (SELECT ID FROM DELETED)
    DELETE CURSO WHERE ID IN (SELECT ID FROM DELETED)
END
GO

CREATE TRIGGER AL_ELIMINAR_ACTIVIDAD ON ACTIVIDAD
INSTEAD OF DELETE
AS
BEGIN
	DELETE REALIZA WHERE ID_ACTIVIDAD = (SELECT ID FROM DELETED)
	DELETE CUMPLE_CON WHERE ID_ACTIVIDAD = (SELECT ID FROM DELETED)
	DELETE ES_DE_TIPO WHERE ID_ACTIVIDAD = (SELECT ID FROM DELETED)
	DELETE ACTIVIDAD WHERE ID = (SELECT ID FROM DELETED)
END
GO

CREATE TRIGGER AL_ELIMINAR_ORGANIZACION ON ORGANIZACION
INSTEAD OF DELETE
AS
BEGIN
	DELETE ACTIVIDAD WHERE CONVOCADA_POR = (SELECT ID FROM DELETED)
	DELETE ORGANIZACION WHERE ID = (SELECT ID FROM DELETED)
END
GO

CREATE TRIGGER AL_ELIMINAR_ODS ON ODS
INSTEAD OF DELETE
AS
BEGIN
	DELETE CUMPLE_CON WHERE ID_ODS = (SELECT ID FROM DELETED)
	DELETE ODS WHERE ID = (SELECT ID FROM DELETED)
END
GO

CREATE TRIGGER AL_ELIMINAR_HORA ON HORA
INSTEAD OF DELETE
AS
BEGIN
	DELETE DISPONE_DE
	WHERE DIA = (SELECT DIA FROM DELETED)
	AND HORA_INICIO = (SELECT HORA_INICIO FROM DELETED)
	AND HORA_FIN = (SELECT HORA_FIN FROM DELETED)
